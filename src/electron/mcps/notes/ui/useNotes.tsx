...
export const useNotes = (): UseNotesReturn => {
  ...
  const [deleteTool] = callTool<NoteData>("notes_delete");
  ...
  /**
   * Delete a note by ID.
   * Navigates back to list view after deletion.
   */
  const deleteNote = useCallback(
    async (noteId: string) => {
      try {
        await deleteTool({ noteId });
        log.debug("Note deleted", { noteId });
      } catch (err) {
        log.error("Failed to delete note", { error: String(err) });
      }
    },
    [deleteTool, log]
  );
