 ...
 * Notes MCP Server
 *
 * Main entry point for the notes MCP. Wires together:
 * - App configuration
...
const main = async () => {
  console.log("[Notes] Starting MCP server");
  await app.start();
  console.log("[Notes] MCP server ready on port", PORT);
};

process.on("SIGTERM", () => {
  console.log("[Notes] Received SIGTERM, shutting down...");
  process.exit(0);
});

process.on("SIGINT", () => {
  console.log("[Notes] Received SIGINT, shutting down...");
  process.exit(0);
});

main().catch((error) => {
  console.error("[Notes] Failed to start server", error);
  process.exit(1);
});
